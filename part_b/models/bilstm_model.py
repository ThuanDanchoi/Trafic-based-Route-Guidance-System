...
def build_bilstm_model(input_shape, output_shape=1):
    model = Sequential([
...
def train_bilstm_model(X_train, y_train, X_val, y_val, input_shape, output_shape=1, epochs=100, batch_size=32):
    ...
    callbacks = [
        ...
        ModelCheckpoint(
            os.path.join('saved_models', 'bilstm_model.h5'),
            monitor='val_loss',
            save_best_only=True,
            verbose=1
        ),
...
def evaluate_bilstm_model(model, X_test, y_test, scaler=None):
...
X_train_bilstm, X_val, y_train_bilstm, y_val = train_test_split(
...
bilstm_metrics = evaluate_bilstm_model(model, X_test, y_test, site_scalers[first_site])
...
for site in unique_sites[:3]:
    ...
    site_metrics[site] = evaluate_bilstm_model(model, X_test_site, y_test_site, scaler)